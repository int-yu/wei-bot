...
- 支持插件机制、事件总线、Web 配置和第三方 Python entry point 发现。
- 提供本地 Web 管理后台。
...
内置插件位于 `src/wechat_ai_companion/plugins/`，启停由 `config.yaml` 和后台共同控制。第三方插件可以作为独立 Python 包安装，不需要修改项目核心源码。当前内置：

- `flow_state`：心流插件，等待用户多段消息讲完后再回复。
...
```powershell
python -m pip install -e .\examples\hello_plugin
```

重启主程序后，Web 后台会显示 `hello_plugin`。启用后可在微信发送 `/hello-plugin` 测试。
...
- 心流插件会先缓存普通聊天消息，等用户停顿后再让 AI 判断是否回复；命令和插件命令不会进入心流缓存。
- AI 正常回复会按微信说话方式拆成多条短消息发送；如果发送过程中收到用户新消息，剩余未发送片段会停止，避免继续说旧话。
